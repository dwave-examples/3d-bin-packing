...
from packing3d import (
    ...
    Cases,
    Variables,
    _add_bin_on_constraint,
    _add_boundary_constraints,
    _add_geometric_constraints,
...
class TestPacking3d(unittest.TestCase):
    ...
    def test_variables(self):
        ...
        self.assertEqual(len(self.variables.bin_loc), self.cases.num_cases * self.bins.num_bins)
    ...
    def test_add_geometric(self):
        cqm = dimod.ConstrainedQuadraticModel()
        effective_dimensions = _add_orientation_constraints(cqm, self.variables, self.cases)
        _add_geometric_constraints(cqm, self.variables, self.bins, self.cases, effective_dimensions)
    ...
    def test_add_boundary_constraints(self):
        ...
        vars = Variables(self.cases, bins)
        cqm = dimod.ConstrainedQuadraticModel()
        effective_dimensions = _add_orientation_constraints(cqm, vars, self.cases)
        _add_boundary_constraints(cqm, vars, bins, self.cases, effective_dimensions)
